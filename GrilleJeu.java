...
public class GrilleJeu extends Observable {
	...
	private static int nbLigne = 6; 
	private static int nbColonne = 7; 
	/**
	 ...
	 */
	public boolean coupJoueur(int i , int j, String couleur){
	 ...
	 */
	public void initialise(int [][] grille , int i , int j){
	 ...
	 */
	public int compter(int ligne, int colonne, int dirLigne, int dirColonne, String couleur) {
		int compteur=0;
		while (grille[ligne][colonne].getCouleur() == "VIDE"
	...
	/** verifie si une colonne contient 4 cases de la meme couleur Ã  partir d'une ligne passe en parametre
	 ...
	 public boolean colonneGagnante(int ligne,int colonne) {
	  if(grille[ligne][colonne].getCouleur()=="VIDE"){
	   return false;
	  }
	  else{
